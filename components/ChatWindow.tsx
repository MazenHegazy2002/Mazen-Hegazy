            ...
            <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
              ...
              >
                ...
                {msg.type === MessageType.VOICE && (
                  <div className="flex items-center space-x-3 w-64 py-2 px-1">
                    <button onClick={() => onPlayVoice(msg, isMe ? 'You' : recipient.name, isMe ? (currentUser.avatar || '') : recipient.avatar)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shadow-lg active:scale-90 ${isMe ? 'bg-white/20 hover:bg-white/30' : 'bg-blue-500 hover:bg-blue-400 text-white'}`}>
                      {isPlaying ? (
                    ...
                    <div className="flex-1 flex items-center justify-between h-8 px-1">
                      {/* Artificial Waveform Visualization */}
                      {[...Array(20)].map((_, i) => {
                        // Generate random-ish heights based on index to look like a voice wave
                        const height = isPlaying
                          ? 30 + Math.random() * 50
                          : 20 + (Math.sin(i) * 20 + Math.cos(i * 3) * 20 + 40) * 0.5;

                        // Animate the bars when playing
                        return (
                          <div
                            key={i}
                            className={`w-1 rounded-full transition-all duration-300 ${isMe ? 'bg-white/40' : 'bg-blue-400'}`}
                            style={{
                    ...
                    <span className="text-[10px] font-bold opacity-60 tabular-nums">
                      {isPlaying ? 'Playing...' : (msg.duration ? `${Math.floor(msg.duration / 60)}:${(msg.duration % 60).toString().padStart(2, '0')}` : 'Voice')}
                    </span>
                  </div>
                )}
